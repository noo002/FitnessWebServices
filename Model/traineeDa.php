...
class traineeDa {
    ...
    public function getTotalTrainee() {
        $conn = Connection::getInstance();
        $sqlSelected = "call getTotalTrainee()";
        $stmt = $conn->getDb()->prepare($sqlSelected);
        try {
            $stmt->execute();
            foreach ($stmt->fetch() as $row) {
    ...
    public function updateAddress($address, $traineeId) {
        $conn = Connection::getInstance();
        $sqlSelected = "call updateAddress(?,?)";
        $stmt = $conn->getDb()->prepare($sqlSelected);
    ...
    public function getRanking($activityId) {
        $conn = Connection::getInstance();
        $sqlSelected = "call getRanking(?)";
        $stmt = $conn->getDb()->prepare($sqlSelected);
        ...
        try {
            ...
            foreach($stmt->fetchAll() as $row){
                $a = array(
                    "traineeId" =>$row['traineeId'],
                    "caloriesBurn" =>$row['totalCaloriesBurn']
                );
                array_push($result,$a);
            }
            return $result;
